  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#07080b' }}>
      ...
      <header className="sticky top-0 z-50"
        style={{ borderBottom: '1px solid #21262d', background: '#07080b' }}>
        <div className={`${wide ? 'max-w-screen-2xl' : 'max-w-4xl'} mx-auto px-5 h-14 flex items-center gap-4`}>

          {/* Logo */}
          <NavLink to="/" className="flex items-center gap-2.5 shrink-0">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center"
              style={{ background: 'rgba(56,139,253,0.12)', border: '1px solid rgba(56,139,253,0.4)' }}>
              <Eye size={14} style={{ color: '#388bfd' }}/>
            </div>
            <div className="leading-none">
              <div className="font-bold text-[14px] tracking-wide" style={{ color: '#c9d1d9', fontFamily: 'Poppins, sans-serif' }}>
                Drishya
              </div>
              <div className="text-[8px] tracking-wider uppercase font-medium mt-px" style={{ color: '#484f58' }}>
                Indian Railways · eGov DIGIT
              </div>
            </div>
          </NavLink>

          <div className="w-px h-5 mx-1" style={{ background: '#21262d' }}/>
          ...
          <nav className="flex items-center">
            {[
              ...
              <NavLink key={n.to} to={n.to} end={n.end}
                className={({ isActive }) =>
                  `flex items-center gap-1.5 px-3 h-14 font-mono uppercase tracking-wider text-[10px] border-b-2 transition-colors ${isActive
